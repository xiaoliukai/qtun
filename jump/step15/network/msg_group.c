...
{
    msg_t* msg;
    if (len > qtun->max_length) return 0;
    msg = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_t) + len);
    if (msg == NULL) return 0;
    g->elements = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_t*));
    ...
    if (g)
    {
        if (g->elements)
        {
            ...
            for (i = 0; i < g->count; ++i)
            {
                ...
                group_pool_room_free(&qtun->group_pool, g->elements[i]);
            }
            group_pool_room_free(&qtun->group_pool, g->elements);
        }
        g->elements = NULL;
        g->count = 0;
        group_pool_room_free(&qtun->group_pool, g);
    }
}
...
msg_group_t* new_msg_group(const void* data, const unsigned short len)
{
    ...
    ret = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_group_t));
    if (ret == NULL) goto end;
    ret->ident = ++qtun->msg_ident;
    if (dst_len <= qtun->max_length)
    {
    ...
    else
    {
        ...
        unsigned short len;
        unsigned char pfx;
        unsigned short fixed = qtun->max_length;
        unsigned short mixed = fixed >> 3;
        msg_t* msg;
        ret->count = (unsigned short)ceil((double)dst_len / fixed);
        ret->elements = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_t*) * ret->count);
        ...
        {
            msg = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_t) + fixed);
        ...
        msg = group_pool_room_alloc(&qtun->group_pool, sizeof(msg_t) + left);
        if (msg == NULL) goto end;
...
void checkout_ttl(hash_t* h)
{
    ...
    while (!hash_is_end(iter))
    {
        ...
        if (qtun->msg_ttl - group->ttl_start > MSG_MAX_TTL)
        {
